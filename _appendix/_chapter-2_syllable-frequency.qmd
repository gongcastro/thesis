...
syllables_data <- read_csv(here("data", "02-chapter-2",
					   "data", "syllables_data.csv"))

childes <- read_csv(here("data", "02-chapter-2",
						 "data", "childes.csv"))

model_fit_syllables <- readRDS(here("data", "02-chapter-2",
									"results", "fits", "fit_syllables.rds"))

posterior_syllables_summary <- read_csv(here("data", "02-chapter-2",
											 "results", "posterior", "posterior_syllables_summary.csv"))
...
pool_tmp <- bvq_data$pool |>
	...
	tidyr::drop_na(xsampa, wordbank_lemma) |>
...
#| tbl-cap: "Posterior distribution of regression coefficients. $\\beta$: median of the posterior distribution in the probability scale. 95\\% HDI: 95\\% highest density interval of the distribution. $p(\\text{ROPE})$: overlap between the 95\\% HDI and the ROPE, indicating the posterior probability that the true value of the coefficient is equivalent to zero."
#| echo: false
#| message: false
#| warning: false
...
syll_plot_data <- expand.grid(
	...
	mutate(lv_std = (lv_std * sd(syllables_data$lv)) + mean(syllables_data$lv),
